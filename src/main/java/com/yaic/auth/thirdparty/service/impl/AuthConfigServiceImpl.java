...
public class AuthConfigServiceImpl implements AuthConfigService {
	...
	@Override
	public List<AccountModel> getAccountTrees(AccountModel accountModel) throws Exception {
		List<AccountModel> lists = accountService.getAccountInfoByCodeOrName(accountModel);
		return lists;
	}
	...
	public List<DataSourceModel> getDatasourceTrees(String deviceCode) throws Exception{
		...
		if(model != null && StringUtils.isNotBlank(model.getAppId()))
	...
	public DataSourceDto getFullDatasourceInfo(DataSourceModel model)  throws Exception{
		...
		//根据APPID获取渠道对应的账户信息
		AccountModel accountModel = accountService.getOneByAppid(model.getAppId());
	...
	public Integer saveDatasourProject(AuthBasicInfo basicInfo) throws Exception {
		...
		if(basicInfo.getAddRole().equals("datasource")){
			...
			if(result > 0){
				...
				model2.setAuthParam2(basicInfo.getAuthParam2());
				model2.setEncryptType(basicInfo.getEncryptType());
				model2.setEncryptParam1(basicInfo.getEncryptParam1());
				model2.setEncryptParam2(basicInfo.getEncryptParam2());
				model2.setAppId(accoModel.getAppId());
			...
			if(result > 0){
				...
				model1.setProjectCode(basicInfo.getDataSource());
				model1.setProjectName(PublicParamters.DEFAULT_PROJECT_NAME);
				model1.setIsDefault(PublicParamters.DEFAULT_PROJECT_Y);
				model1.setAccountId(accoModel.getAccountId());
			...
			//根据渠道编号获取渠道信息
			DataSourceModel datasourceModel = dataSourceService.getOneByDataSourceId(basicInfo.getPrimaryId());
			
			AccountModel accoModel1 = accountService.getOneByAppid(datasourceModel.getAppId());
			...
			model2.setEncryptParam1(basicInfo.getEncryptParam1());
			model2.setEncryptParam2(basicInfo.getEncryptParam2());
			model2.setAppId(datasourceModel.getAppId());
			model2.setCreatedUser(loginUserCode);
			model2.setUpdatedUser(loginUserCode);
			result = authEncryptService.addInfo(model2);
			...
			if(result > 0){
				...
				model.setIsDefault(PublicParamters.DEFAULT_PROJECT_N);
				model.setAccountId(accoModel1.getAccountId());
